...
class SCTSolver:
    ...
    def createSpeciesCompositionGraph(self, parser, configurationFile, namingConventions,
        ...
        # classify reactions
        self.database.classifications, equivalenceTranslator, self.database.eequivalenceTranslator,\
        ...
        self.database.translator, self.database.userLabelDictionary, \
            self.database.lexicalLabelDictionary, self.database.partialUserLabelDictionary = self.database.sbmlAnalyzer.getUserDefinedComplexes()
        ...
        for reaction, classification in zip(rules, self.database.classifications):
            self.bindingReactionsAnalysis(self.database.dependencyGraph,
        ...
        for element in lexicalDependencyGraph:
            ...
            if element in self.database.dependencyGraph and element not in self.database.userLabelDictionary:
            ...
            for dependencyCandidate in self.database.dependencyGraph[element]:
                for molecule in [x for x in dependencyCandidate if x not in self.database.dependencyGraph]:
        ...
        for element in self.database.userLabelDictionary:
            ...
            elif len(self.database.userLabelDictionary[element][0]) == 0 or element == \
                    self.database.userLabelDictionary[element][0][0]:

                self.database.dependencyGraph[element] = []
            else:
                self.database.dependencyGraph[element] = [list(
                    self.database.userLabelDictionary[element][0])]
                # If the user is introducing a new molecule term, add it to the SCT
                if self.database.userLabelDictionary[element][0][0] not in self.database.dependencyGraph:
                    self.database.dependencyGraph[
                        self.database.userLabelDictionary[element][0][0]] = []
        ...
        molecules.extend(['{0}()'.format(
            x) for x in self.database.userLabelDictionary if '{0}()'.format(x) not in molecules])
        # recalculate 1:1 equivalences now with binding information
        _, _, self.database.eequivalenceTranslator2,\
            _, adhocLabelDictionary, _, _ = self.database.sbmlAnalyzer.classifyReactions(
        ...
        for key in self.database.eequivalenceTranslator2:
            for namingEquivalence in self.database.eequivalenceTranslator2[key]:
                ...
                if key != 'Binding' and modElement not in self.database.userLabelDictionary:
                    if baseElement not in self.database.dependencyGraph:
                        self.database.constructedSpecies.add(baseElement)
                        self.database.dependencyGraph[baseElement] = []
                    if modElement not in self.database.dependencyGraph or not [True for x in self.database.dependencyGraph[modElement] if baseElement in x and len(x) > 1]:
                        if baseElement in self.database.annotationDict and modElement in self.database.annotationDict:
                            baseSet = set([y for x in self.database.annotationDict[baseElement] for y in self.database.annotationDict[baseElement][x]])
                            modSet = set([y for x in self.database.annotationDict[
                                          modElement] for y in self.database.annotationDict[modElement][x]])
                            if len(baseSet.intersection(modSet)) > 0 or len(baseSet) == 0 or len(modSet) == 0:
                                if modElement not in self.database.dependencyGraph:
                                    # if the entry doesnt exist from previous information accept this
                                    atoAux.addToDependencyGraph(self.database.dependencyGraph, modElement, [baseElement])
                                else:
                                    # otherwise add it to the lexical repository
                                    atoAux.addToDependencyGraph(self.database.alternativeDependencyGraph, modElement, [baseElement])
                            else:
    ...
    choosing binding'.format(self.database.dependencyGraph[modElement], baseElement, modElement))
            ...
            for reaction, classification in zip(rules, self.database.classifications):
                ...
                if len(preaction[0]) == 1 and len(preaction[1]) == 1:
                    ...
                    if preaction[1][0].lower() in preaction[0][0].lower() or len(preaction[1][0]) < len(preaction[0][0]):
                        ...
                        mod = preaction[1][0]
                        base = preaction[0][0]
                    if self.database.dependencyGraph[mod] == [] and mod not in self.database.userLabelDictionary:

                        if base in self.database.userLabelDictionary and \
                                self.database.userLabelDictionary[base] == 0:
                            continue
                        if mod in self.database.userLabelDictionary and \
                                self.database.userLabelDictionary[mod] == 0:
                            continue
                        ...
                        # this modifications
                        if base in self.database.annotationDict and mod in self.database.annotationDict:
                            baseSet = set([y for x in self.database.annotationDict[
                                          base] for y in self.database.annotationDict[base][x]])
                            modSet = set(
                                [y for x in self.database.annotationDict[mod] for y in self.database.annotationDict[mod][x]])
    ...
    '''
        ...
        for element in self.database.lexicalLabelDictionary:
            ...
            # before
            if element in self.database.dependencyGraph and \
                    len(self.database.dependencyGraph[element]) > 0:
                continue

            if len(self.database.lexicalLabelDictionary[element][0]) == 0 or element == \
                    self.database.lexicalLabelDictionary[element][0][0]:
                self.database.constructedSpecies.add(element)
                ...
                self.database.dependencyGraph[element] = [list(
                    self.database.lexicalLabelDictionary[element][0])]
        ...
        orphanedSpecies = [
            x for x in strippedMolecules if x not in self.database.dependencyGraph or self.database.dependencyGraph[x] == []]
        orphanedSpecies.extend([x for x in self.database.dependencyGraph if self.database.dependencyGraph[
        ...
        for annotatedSpecies in annotationDependencyGraph:
            if len(annotationDependencyGraph[annotatedSpecies]) > 0 and annotatedSpecies not in self.database.userLabelDictionary:
                atoAux.addToDependencyGraph(
                    self.database.dependencyGraph, annotatedSpecies, annotationDependencyGraph[annotatedSpecies][0])
        ...
        annotationDependencyGraph, _ = self.fillSCTwithAnnotationInformation(
            nonOrphanedSpecies, self.database.annotationDict, self.database,tentativeFlag=True)
        ...
        strippedMolecules.extend([x.strip('()') for x in self.database.constructedSpecies])
        ...
        tmpDependency, self.database.tmpEquivalence = self.database.sbmlAnalyzer.findClosestModification(
            orphanedSpecies, strippedMolecules, self.database.annotationDict, self.database.dependencyGraph)

        for species in tmpDependency:
            if species not in self.database.userLabelDictionary:
                if tmpDependency[species] == []:
                    atoAux.addToDependencyGraph(self.database.dependencyGraph, species, [])
                for instance in tmpDependency[species]:
                    atoAux.addToDependencyGraph(self.database.dependencyGraph, species, instance)
                    if len(instance) == 1 and instance[0] not in self.database.dependencyGraph:
                        atoAux.addToDependencyGraph(self.database.dependencyGraph, instance[0], [])
        ...
        for reactant in orphanedSpecies:
            ...
            if greedyMatch not in [-1,-2, []]:
                atoAux.addToDependencyGraph(self.database.dependencyGraph, reactant, greedyMatch)
        ...
        if len(self.database.constructedSpecies) > 0:
            logMess('INFO:SCT031', 'The following species names do not appear in the original model but where created to have more appropiate naming conventions: [{0}]'.format(','.join(self.database.constructedSpecies)))

        # initialize and remove zero elements
        self.database.prunnedDependencyGraph, self.database.weights, unevenElementDict, self.database.artificialEquivalenceTranslator = \
            self.consolidateDependencyGraph(self.database.dependencyGraph, equivalenceTranslator,
                                       self.database.eequivalenceTranslator, self.database.sbmlAnalyzer)
    ...
    def fillSCTwithAnnotationInformation(self, orphanedSpecies, annotationDict, logResults=True,tentativeFlag=False):
            # annotation handling
        ...
        exactMatches = self.consolidateDependencyGraph(
            dict(exactMatches), {}, {}, self.database.sbmlAnalyzer, loginformation=False)[0]
        ...
        if logResults:
            for x in [y for y in exactMatches if len(exactMatches[y]) > 0]:
                if not tentativeFlag:
                    ...
                    if not (x in self.database.dependencyGraph and exactMatches[x][0][0] in self.database.dependencyGraph and self.database.dependencyGraph[x] == self.database.dependencyGraph[exactMatches[x][0][0]]):
                        logMess('WARNING:ANN101', '{0}:{1} were determined to be equivalent according to annotation information. Please confirm from user information'.format(
                        x, exactMatches[x][0][0]))
        ...
        strongIntersectionMatches = self.consolidateDependencyGraph(dict(strongIntersectionMatches), {}, {}, self.database.sbmlAnalyzer, loginformation=False)[0]
        if logResults:
            for x in [y for y in strongIntersectionMatches if len(strongIntersectionMatches[y]) > 0]:
                if x not in exactMatches:
                    if not tentativeFlag:
                        ...
                        if not(x in self.database.dependencyGraph and strongIntersectionMatches[x][0][0] in self.database.dependencyGraph and \
                            self.database.dependencyGraph[x] == self.database.dependencyGraph[strongIntersectionMatches[x][0][0]]):
    ...
    def consolidateDependencyGraph(self, dependencyGraph, equivalenceTranslator,
                                   equivalenceDictionary, sbmlAnalyzer, loginformation=True):
        """
        ...
        def selectBestCandidate(reactant, candidates, dependencyGraph, sbmlAnalyzer,
            ...
            for individualAnswer in candidates:
                try:
                    ...
                    if loginformation:
                        logMess('ERROR:SCT221', '{0}:{1}:Dependency cycle found when mapping molecule to candidate'.format(reactant, individualAnswer[0]))
                    continue
            # we cannot handle tuple naming conventions for now
    ...
    this the correct behavior or provide an alternative for {0}'.format(reactant, tmpCandidates[0], lexCandidateModification, lexCandidate))
                    ...
                    if fuzzyCandidateMatch is not None:
                        ...
                        tmpDependency, tmpEquivalence = sbmlAnalyzer.findClosestModification(
                            [reactant], dependencyGraph.keys(), self.database.annotationDict, self.database.dependencyGraph)
                        ...
                        for individualCandidate in tmpCandidates:
                            for tmpCandidate in individualCandidate:
                                activeQuery = None
                                uniprotkey = atoAux.getURIFromSBML(tmpCandidate,self.database.parser,  ['uniprot'])
                                if len(uniprotkey) > 0:
                            ...
                            for tmpCandidate, candidate in zip(tmpCandidates, candidates):
                                ...
                                if len(fuzzyList) > 0 and fuzzyList[0][1]:
                                    if sbmlAnalyzer.testAgainstExistingConventions(fuzzyList[0][1], sbmlAnalyzer.namingConventions['modificationList']):
                                        self.database.eequivalenceTranslator2[fuzzyList[0][1]].append((activeCandidates[0], '{0}{1}'.format(activeCandidates, fuzzyList[0][1])))
                                    else:
                                        self.database.eequivalenceTranslator2[fuzzyList[0][1]] = [(activeCandidates[0], '{0}{1}'.format(activeCandidates[0], fuzzyList[0][1]))]
                ...
                if all(sorted(x) == sorted(tmpCandidates[0]) for x in tmpCandidates):
                    ...
                    if not all(sorted(x) == sorted(originalTmpCandidates[0]) for x in originalTmpCandidates):
                        if loginformation:
                            logMess('INFO:SCT001', '{0}:Using lexical analysis since stoichiometry gives non-consistent information naming({1})!=stoichiometry({2})'.format(reactant,
                                                                                                                                                               self.database.alternativeDependencyGraph[reactant][0], tmpCandidates))
                    ...
                    if not any([sorted(subcandidate) == sorted(namingTmpCandidates[0]) for subcandidate in tmpCandidates]):
                        ...
                        atoAux.addAssumptions('lexicalVsstoch', (reactant, ('lexical', str(
                            namingTmpCandidates)), ('stoch', str(tmpCandidates)), ('original', str(originalTmpCandidates))), self.database.assumptions)
                    ...
                    atoAux.addAssumptions('lexicalVsstoch', (reactant, ('current', str(replacementCandidate)), ('alternatives', str(
                        [x for x in tmpCandidates if x != replacementCandidate[0]])), ('original', str(originalTmpCandidates))), self.database.assumptions)
                    tmpCandidates = replacementCandidate
                else:
                    tmpCandidates2 = [x for x in tmpCandidates if all(y not in x for y in self.database.constructedSpecies)]
                ...
                if tmpCandidates[0] != self.database.alternativeDependencyGraph[reactant][0]:
                    ...
                    if namingtmpCandidates and tmpCandidates[0] != namingtmpCandidates[0]:
                        ...
                        if loginformation:
                            if namingtmpCandidates[0][0] in self.database.constructedSpecies:
                                namingTmpCandidates = tmpCandidates

                            else:
                                self.database.alternativeDependencyGraph[reactant] = namingtmpCandidates
                                logMess('WARNING:SCT111', '{0}:stoichiometry analysis:{1}:conflicts with and naming conventions:{2}:Selecting lexical analysis'.format(
                                        reactant, tmpCandidates, self.database.alternativeDependencyGraph[reactant]))
                        tmpCandidates = namingtmpCandidates
                        atoAux.addAssumptions('lexicalVsstoch', (reactant, ('stoch', str(tmpCandidates)), ('lexical', str(
                            namingtmpCandidates)), ('original', str(originalTmpCandidates))), self.database.assumptions)
        ...
        for element in self.database.alternativeDependencyGraph:
            if element in tempMergedDependencyGraph:
                tempMergedDependencyGraph[element].extend(self.database.alternativeDependencyGraph[element])
